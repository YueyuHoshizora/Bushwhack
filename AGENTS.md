...
9. **本機儲存**：`localStorage` 鍵為 `bushwhack-profile`（紀錄含最佳撤離波次、累計值、擊倒過的首領種類、成就、難度、兵種、外觀、突變）、`bushwhack-music`／`bushwhack-sfx` 與其 `-volume`。修改 `profile` 結構時要相容舊資料（缺欄位補預設值、未解鎖的兵種與外觀退回預設）。
