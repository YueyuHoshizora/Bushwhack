...
// Personal records, cumulative totals, unlocked achievements and the chosen difficulty, class and mutators, kept in localStorage.
...
const profile = (() => {
  ...
  return { records: data.records ?? {}, daily: data.daily ?? null, totals: { ambushKills: 0, bossKills: 0, ...data.totals }, achievements: Array.isArray(data.achievements) ? data.achievements : [], difficulty: data.difficulty ?? 'normal', cls: CONFIG.classes[data.cls] ? data.cls : 'ranger', mutators };
})();
...
// A run is either a normal game (random seed, chosen difficulty/mutators, achievement rewards) or today's daily challenge (seeded mutator).
function startGame(daily) {
  sound.init();
  const seed = daily ? todayUTC() : `${Date.now()}:${Math.random()}`, T = CONFIG.throwables;
  ...
  game.boss = null; game.event = null; game.perks = {}; game.perkOffer = null; game.route = null; game.nextRoute = null; game.routeOffer = null; game.seenAffixes = new Set(); game.newAchievements = [];
  game.stats = { damage: {}, ambushKills: 0, takedowns: 0, taken: 0, bosses: 0, perks: [] };
...
function gunStats(key = game.player.weapon) {
  const l = game.player.levels, g = CONFIG.gun, w = CONFIG.weapons[key], M = CONFIG.mods;
  return {
    ...w, damage: Math.round(w.damage * (1 + l.damage * g.damageStep) * (1 + perk('glassCannon') * P.glassCannon.damage)),
    shotsPerSecond: w.shotsPerSecond * (1 + l.rate * g.rateStep) * (1 + perk('trigger') * P.trigger.rate + perk('frenzy') * P.frenzy.rate),
    pellets: w.pellets + l.spread, range: Math.round(w.range * (1 + l.range * g.rangeStep)), pierce: w.pierce + l.piercing * M.piercing.pierce,
    bounces: l.ricochet * M.ricochet.bounces, noise: w.noise * (1 - l.suppressor * M.suppressor.noise)
  };
}
function gearStats() {
...
// The choice overlay shows either the perk pick or the route pick that follows it.
function renderChoice() {
  const perks = game.mode === 'perk', keys = perks ? game.perkOffer : game.routeOffer;
  $('perkWave').textContent = String(game.wave).padStart(2, '0');
  $('perkLabel').textContent = t(perks ? 'perk.eyebrow' : 'route.eyebrow');
  $('perkHeading').textContent = t(perks ? 'perk.heading' : 'route.heading');
  $('perkHint').textContent = t(perks ? 'perk.hint' : 'route.hint', { count: keys.length });
  $('perkChoices').replaceChildren(...keys.map((key, i) => {
    const item = perks ? P[key] : CONFIG.routes.list[key], button = document.createElement('button');
    button.type = 'button'; button.className = perks ? `perk-card rarity-${item.rarity}` : 'perk-card';
    const note = perks ? `${t(`rarity.${item.rarity}`)} · ${t('perk.stack', { level: perk(key) + 1, max: item.max })}` : t('route.next', { wave: game.wave + 1 });
    const text = perks ? t(`perk.${key}.desc`, perkVars(key)) : t(`route.${key}.desc`, routeVars(key));
    button.innerHTML = `<span class="keycap">${i + 1}</span><span class="perk-icon">${item.icon}</span><b>${t(`${perks ? 'perk' : 'route'}.${key}.title`)}</b><small>${note}</small><span>${text}</span>`;
    button.addEventListener('click', () => choose(i));
    return button;
  }));
}
function choose(index) { if (game.mode === 'perk') choosePerk(index); else if (game.mode === 'route') chooseRoute(index); }
function choosePerk(index) {
  ...
  game.perkOffer = null;
  burst(p.x, p.y, '#e9e597', 17); sound.play('buy'); notify(t('toast.perk', { name: t(`perk.${key}.title`) })); updateHUD();
...
function classVars(key) {
  ...
  return { speed: pct(c.speed), scrap: pct(c.scrap), hp: c.hp, cooldown: s.cooldown, distance: s.distance, max: s.max, seconds: s.seconds, reduction: pct(s.reduction), damage: s.damage };
}
...
// Start screen: difficulty, class and mutator pickers (locked tiers name their achievement), best record, today's daily and achievements.
function renderMenu() {
  ...
  $('difficultyPicker').replaceChildren(...Object.entries(CONFIG.difficulty).map(([key, d]) => {
    ...
    return pickerButton(t(`difficulty.${key}`), key === profile.difficulty, open ? t(`difficulty.${key}.desc`) : t('menu.locked', { name: t(`achievement.${d.unlock}.title`) }), () => { profile.difficulty = key; saveProfile(); renderMenu(); }, !open);
  }));
  $('recordLine').textContent = record ? t('menu.record', { wave: record.wave, kills: record.kills, score: record.score ?? 0 }) : t('menu.noRecord');
  $('classPicker').replaceChildren(...Object.entries(CONFIG.classes).map(([key, c]) => pickerButton(`${c.icon} ${t(`class.${key}`)}`, key === profile.cls, t(`class.${key}.desc`, classVars(key)), () => { profile.cls = key; saveProfile(); renderMenu(); })));
...
function applyLocale(lang) {
  ...
  if (game.mode === 'perk' || game.mode === 'route') renderChoice();
...
function updateHUD() {
  ...
  $('stealthText').textContent = (hidden ? t('hud.hidden') : game.event?.carrying ? t('hud.carrying') : inBush ? t('hud.revealed') : inTerrain(p, game.ponds) ? t('hud.wading') : t('hud.exposed')) + holding;
...
// not lit by a flare and not carrying the intel case.
function isHidden() {
  const p = game.player;
  if (!p || game.time < p.revealedUntil || game.event?.carrying || game.flares.some(f => distance(f, p) < f.radius)) return false;
...
function shoot() {
  ...
  p.revealedUntil = game.time + CONFIG.player.revealSeconds * (1 - p.levels.suppressor * M.suppressor.reveal) * (1 + perk('frenzy') * P.frenzy.reveal);
  for (let i = 0; i < gun.pellets; i++) {
    ...
    game.bullets.push({ x, y, vx: Math.cos(a) * gun.bulletSpeed, vy: Math.sin(a) * gun.bulletSpeed, traveled: 0, range: gun.range, damage, radius: CONFIG.gun.bulletRadius, friendly: true, pierce: gun.pierce, hits: gun.pierce ? new Set() : null, bounces: gun.bounces, color: gun.color, trail: p.weapon === 'rail', source: p.weapon, ambush });
  }
...
function bossDeath(boss) {
  ...
  game.boss = null; game.stats.bosses++; addScore(CONFIG.score.boss);
...
// Shared blast: damages enemies and chests, sets off barrels, optionally hurts the player and burns grass. Every blast makes noise.
...
function renderEnd() {
  ...
  $('endAmbush').textContent = s.ambushKills; $('endTakedowns').textContent = s.takedowns; $('endTaken').textContent = Math.round(s.taken); $('endBosses').textContent = s.bosses;
  ...
  $('endMode').textContent = [game.daily ? `${t('hud.daily')} ${game.daily}` : t(`difficulty.${game.difficulty}`), t(`class.${game.cls}`), t(`variant.${game.variant}`), ...game.mutators.map(key => t(`mutator.${key}`))].join(' · ');
...
function shareText() {
  ...
  return t('share.text', { date: game.daily, wave: run.wave, kills: run.kills, score: run.score, time: formatTime(run.time), variant: t(`variant.${game.variant}`), mutator: t(`mutator.${game.mutators[0]}`), url: SITE_URL });
}
...
function updateScout(e, dt, d) {
  ...
  if (d < s.scan && e.special <= 0) {
    ...
    alertNoise(p.x, p.y, s.alarm); e.special = s.scanCooldown; sound.play('beep', e);
  }
}
...
// Ricochet bullets reflect off the wall face they hit (the axis whose move alone collides) while bounces remain.
...
function takedown() {
  ...
  p.takedownCooldown = CONFIG.takedown.cooldown; game.stats.takedowns++;
...
// Class skill on Space: ranger rolls (invulnerable), engineer plants a mine, heavy braces (damage reduction).
function useSkill() {
...
function update(dt) {
  ...
  p.skillCooldown -= dt; p.takedownCooldown -= dt; p.bulwark -= dt;
  ...
  if (game.waveRemaining > 0) {
    ...
    if (!game.nextWave) {
      ...
      game.route = null;
      if (offerPerks() || offerRoute()) return;
    }
    game.nextWave -= dt;
...
// Screen-edge arrows toward off-screen objectives (hold zone, marked officer, intel case / exit).
function drawPointers(camera, w, h) {
  ...
  ctx.fillStyle = ev.type === 'assassinate' ? '#ff8a6a' : '#f3d182'; ctx.beginPath(); ctx.moveTo(12, 0); ctx.lineTo(-8, -9); ctx.lineTo(-4, 0); ctx.lineTo(-8, 9); ctx.fill();
  ctx.restore();
}
...
window.addEventListener('keydown', event => {
  ...
  else if ((game.mode === 'perk' || game.mode === 'route') && !event.repeat && key >= '1' && key <= '9') choose(Number(key) - 1);
